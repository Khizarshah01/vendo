/**
 * The standalone agent surface — spawn a governed, harness-grade agent in any
 * Node backend in a few lines. One runtime, always host-run; a Vendo Cloud key
 ...
 *
 * This was `@vendoai/agents`' barrel until that package folded into the
 * umbrella. It is re-exported verbatim from `../index.js`, so every name below
 * is still reachable under its own name from `@vendoai/vendo`.
 */
export {
  agent,
  agentComposition,
