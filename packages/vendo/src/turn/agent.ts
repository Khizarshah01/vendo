...
const resolveSandbox = (explicit: SandboxAdapter | undefined): SandboxAdapter => {
  ...
  if (cloudKey() !== undefined) {
    throw new VendoError(
      ...
      + "(import `e2b` from `@vendoai/vendo`) — or use a build that ships the Cloud sandbox rung "
  ...
  throw new VendoError(
    ...
    + "E2B_API_KEY as its credential (import `e2b` from `@vendoai/vendo`) — or set VENDO_API_KEY "
