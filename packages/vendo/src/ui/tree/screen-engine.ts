 ...
 *
 * `bootScreen` / `flattenTree` live behind `@vendoai/vendo/core/apps` — the
 * browser-safe door `@vendoai/vendo/ui` is allowed to reach (enforced in
 ...
 *    `@vendoai/vendo/core`, which `@vendoai/vendo/ui` DEPENDS on — so the old reason (two
 ...
 *    `@vendoai/vendo/core` — a pin, or a second copy another dependency hoisted — gets
 ...
 *  (`vm-program.ts`) are the same law — this is the copy `@vendoai/vendo/ui` may hold,
...
export const loadScreenEngine = async (): Promise<ScreenEngine> => {
  const door = await import("../../core/apps/index.js") as unknown as Partial<ScreenEngine> & {
    warmScreenEngine?: () => Promise<void>;
  };
  if (typeof door.bootScreen !== "function" || typeof door.flattenTree !== "function") {
    throw new Error("this build of @vendoai/vendo/core carries no screen engine");
  }
