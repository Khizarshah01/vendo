 ...
 */
import type { Decisions, Interruption, Json } from "../core/index.js";
...
/** Written out rather than imported, for the reason `use-vendo-thread.ts` (its
    ...
    wire that stamps it, and @vendoai/vendo/ui may depend on core and apps alone
  ...
  const resume = useCallback(
    async (decisions: Decisions): Promise<void> => {
      for (const approve of [true, false]) {
        ...
        // (packages/vendo/src/core/turn-result.ts) — so an `{ answers }` decision has
