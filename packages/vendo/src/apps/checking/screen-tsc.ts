 ...
 * and this module is reachable from the Worker server entry, so a static
 * `import ... from "typescript"` here would break `pnpm lint`. It is a separate
 * loader from
...
export function screenProgram(input: ScreenTscInput): ScreenProgram {
  ...
  if (ts === null) {
    return { ok: false, why: "no usable TypeScript compiler is reachable from @vendoai/vendo" };
  }
