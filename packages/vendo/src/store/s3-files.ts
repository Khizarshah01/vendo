...
export function s3Files(options: S3FilesOptions): FilesAdapter {
  ...
  return {
    async put(key, bytes, meta) {
      await call("PUT", key, {
        // `as BodyInit`, the same cast wire/apps.ts:355 makes: the DOM lib's
        // BufferSource is ArrayBufferView<ArrayBuffer>, and a plain Uint8Array
        // is ArrayBufferView<ArrayBufferLike> — assignable at runtime, not to
        // the compiler.
