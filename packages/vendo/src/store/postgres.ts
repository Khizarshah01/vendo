/** @vendoai/vendo/store/postgres — the same store on a real Postgres, with the
...
export function createStore(config: PostgresStoreConfig): VendoStore {
  if (!config.url) {
    throw new VendoError(
      "validation",
      "@vendoai/vendo/store/postgres needs a Postgres connection string — pass { url }. "
        + 'For the zero-config PGlite dev default, import createStore from "./index.js" instead.',
    );
  }
