...
export default async function loadWasm(): Promise<ArrayBuffer> {
  ...
  if (!response.ok) throw new Error(`the screen engine's WebAssembly answered ${response.status} — @vendoai/vendo/core ships quickjs.wasm beside its dist, and this host's bundler did not emit it`);
