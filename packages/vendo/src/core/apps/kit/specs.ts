 ...
 * `@vendoai/vendo/ui`'s `KIT_COMPONENTS`, keyed by these names (a ui drift test
 ...
 * a `ReactNode` prop it actually paints (`@vendoai/vendo/ui`). Teaching a slot the Kit
 ...
 * table itself. `@vendoai/vendo/ui`'s `test/kit/slot-drift.test.tsx` puts a probe in
 ...
 * (`packages/vendo/src/ui/tree/renderer.tsx` `builtinContent`), so a chart handed a
...
/** The same list, as the mutable array `@vendoai/vendo/ui`'s registry wants. */
