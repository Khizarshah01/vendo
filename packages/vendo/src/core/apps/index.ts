/**
 * `@vendoai/vendo/core/apps` — the app format, browser-safe.
 *
 ...
 * this door is importable from a browser bundle, which is why `@vendoai/vendo/ui`
 ...
 * (`@vendoai/vendo/apps`), which is node-only.
 */
// app-document — the envelope. It STAYS in `@vendoai/vendo/core`: core's own store
...
export type { PathBinding, ReshapeStep, StateBinding, TreeNode, UIPayload } from "../index.js";
...
// Not part of the Kit's model-facing surface above — a plain zod-schema
// describer. It is on the door because it has TWO readers and S11d put them in
// different packages: ./kit/catalog-prompt.ts here, and the screen type printer
// in @vendoai/vendo (src/apps/checking/screen-typings.ts).
export { zodShape, type ZodKind, type ZodShape } from "./kit/zod-shape.js";
...
// the wire shapes `/apps/*` returns, which @vendoai/vendo/ui re-exports
