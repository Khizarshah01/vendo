 ...
 *  table bundles.
 *
 *  The two `@vendoai/ui` spellings are the names the fold retired, kept for the
 *  same reason the never-real ones below are here: this list is READ against
 *  source a model wrote, and a model writes what the docs it was trained on
 *  said. Dropping them turns a screen that used to render into an unresolvable
 *  import. */
