...
import { initTelemetry } from "../../src/telemetry/edge.js";
...
describe("edge telemetry entry", () => {
  ...
  it("keeps the module free of node builtin imports", async () => {
    ...
    const source = await readFile(new URL("../../src/telemetry/edge.ts", import.meta.url), "utf8");
    expect(source).not.toMatch(/from "node:/);
    expect(source).not.toMatch(/require\(/);
  });

  it("reads as opted out with nothing persisted (no disk on the edge)", async () => {
    const { loadConfig } = await import("../../src/telemetry/edge.js");
  ...
  it("reports no git remote host (deployed bundles have no working copy)", async () => {
    const { repoHost } = await import("../../src/telemetry/edge.js");
  ...
  it("shares the pure consent module with the Node build", async () => {
    const edge = await import("../../src/telemetry/edge.js");
    const node = await import("../../src/telemetry/consent.js");
