 ...
 *
 * The engine behind the seam is a double, and since S11d that is a CHOICE
 * rather than a compiler's verdict: the fold put the app-generation code and
 * the automations engine in one package, so nothing stops an import any more.
 * The double is the seam's only remaining enforcement. Everything on THIS side of it is real — the real tool registry, the
