...
} from "../../../src/apps/checking/toolchain.js";
...
describe("component runs the ONE gauntlet, and it is the paint gate", () => {
  ...
  it("marks a toolchain that could not RUN as the deployment's fault, remedy and all", async () => {
    ...
    const why = "no esbuild is reachable from @vendoai/vendo's own installation — it is a dependency"
      + " of that package, so reinstall your dependencies, then restart";
