 ...
 * a shortcut.
 *
 * `@vendoai/vendo/ui` and `@vendoai/vendo/apps` are separately published packages, so a host
...
const loadWith = async (door: Record<string, unknown>): Promise<typeof import("../../../src/ui/tree/screen-engine.js")> => {
  vi.resetModules();
  vi.doMock("../../../src/core/apps/index.js", () => door);
  return import("../../../src/ui/tree/screen-engine.js");
};

afterEach(() => {
  vi.doUnmock("../../../src/core/apps/index.js");
  vi.resetModules();
});
...
describe("loadScreenEngine", () => {
  it("says the build carries no screen engine, rather than a TypeError on the first click", async () => {
    ...
    await expect(loadScreenEngine()).rejects.toThrow("this build of @vendoai/vendo/core carries no screen engine");
  });
