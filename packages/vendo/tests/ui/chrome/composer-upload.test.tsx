...
import { VendoOverlay } from "../../../src/ui/chrome/index.js";
...
describe("the composer saves a file and sends the reference", () => {
  ...
  it("uploads a spreadsheet and puts its path on the turn, not its bytes", async () => {
    ...
    await send(new File([csv], "sales 2026.csv", { type: "text/csv" }), "make me a dashboard");
    ...
    // …carrying the FILE. A body fetch does not recognize as a Blob is sent as
    // the string "[object File]", which every other assertion here survives.
    expect(new TextDecoder().decode(Uint8Array.from(uploads()[0]!.body as number[]))).toBe(csv);
