...
import { KIT_COMPONENTS, KIT_SPECS } from "../../../src/ui/kit/registry.js";
import type { KitSlotSpec } from "../../../src/ui/kit/schema.js";
...
describe("every declared slot renders what it promises", () => {
  it("paints a probe put at each slot's declared path", async () => {
    ...
    for (const [component, name, slot] of declared) {
      ...
      expect(context, `${at} is declared in SLOTS with no context here — implement the slot in @vendoai/vendo/ui and add one, or drop it from the table`).toBeTypeOf("object");
