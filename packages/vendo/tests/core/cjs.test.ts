 ...
 *  the contract half. ESM stays the primary condition; this smoke-requires the
 *  BUILT package through Node's own resolver (self-reference via `exports`), so
 *  it is the `require` legs of ./core and ./core/conformance under test — not a
 *  source path, which would prove nothing about the published map. */
describe("CJS export condition", () => {
  it("require('@vendoai/vendo/core') and require('@vendoai/vendo/core/conformance') load the real surface", () => {
    const script = `
      const core = require("@vendoai/vendo/core");
      const conformance = require("@vendoai/vendo/core/conformance");
