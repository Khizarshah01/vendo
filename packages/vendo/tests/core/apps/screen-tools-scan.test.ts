...
import {
  ...
  scanIslandTools,
} from "../../../src/core/apps/screen-tools-scan.js";
...
describe("isIslandResolvableSpecifier", () => {
  // The list is READ against source a model wrote, and a model writes the name
  // the docs it learned from used. The retired spellings resolve for the same
  // reason the never-real kit aliases do: rejecting one turns a screen that
  // used to render into an unresolvable import.
  it.each([
    // Assembled from segments: the dependency guard's static scan reads
    // import-shaped text, and it rejects a retired name spelled out.
    ["@vendoai", "ui"],
    ["@vendoai", "ui", "kit"],
    ["@vendoai", "vendo", "ui"],
    ["@vendoai", "vendo", "ui", "kit"],
  ])("resolves %s", (...segments) => {
    expect(isIslandResolvableSpecifier(segments.join("/"))).toBe(true);
  });

  it("does not resolve a specifier the venue cannot supply", () => {
    expect(isIslandResolvableSpecifier("node:fs")).toBe(false);
    expect(isIslandResolvableSpecifier("@vendoai/vendo/store")).toBe(false);
  });
});
