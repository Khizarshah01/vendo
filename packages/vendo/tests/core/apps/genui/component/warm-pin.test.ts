 ...
 *
 * `@vendoai/vendo/ui` re-warms the engine with no variant of its own the moment a
...
import { bootScreen, warmScreenEngine } from "../../../../../src/core/apps/genui/component/index.js";
...
describe("the variant a host pinned", () => {
  ...
  it("survives the library's own default re-warm, and is what boots the screen", async () => {
    ...
    // What `@vendoai/vendo/ui` calls on every screen mount. It must not reach for the
