...
vi.mock("@electric-sql/pglite", () => {
  throw new Error("@vendoai/vendo/store/postgres must never load @electric-sql/pglite");
});

describe("@vendoai/vendo/store/postgres entry", () => {
  it("imports and constructs a store without PGlite in the module graph", async () => {
    const { createStore } = await import("../../src/store/postgres.js");
