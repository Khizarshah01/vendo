/**
 * Regenerates src/ui/kit/icons.gen.ts — the Icon brick's path data, extracted from
 ...
 *
 * Run: pnpm --filter @vendoai/vendo build:icons
 * Check (writes nothing, exits 1 when the committed file is stale):
 *   pnpm --filter @vendoai/vendo check:icons
 */
...
const output = resolve(here, "../src/ui/kit/icons.gen.ts");
// The NAMES are model-facing vocabulary — the catalog prompt lists them so the
// model stops inventing glyphs — and `@vendoai/vendo/core` cannot import `@vendoai/vendo/ui`
...
const namesOutput = resolve(here, "../src/core/apps/kit/icon-names.gen.ts");
 ...
 * lives in \`@vendoai/vendo/ui\`'s icons.gen.ts, which this package may not import.
 *
 * Regenerate: pnpm --filter @vendoai/vendo build:icons
 */
...
for (const [path, contents] of files) {
  if (process.argv.includes("--check")) {
    if (await readFile(path, "utf8") !== contents) {
      console.error(`${path} is stale — run: pnpm --filter @vendoai/vendo build:icons`);
      process.exit(1);
    }
