 ...
 * a component screen is checked against (packages/vendo/src/apps/checking/component-screen.ts:534) —
 * keeps that bundle to what the screen actually needs.
 *
 * Run from `packages/vendo`, after `pnpm install`:
 ...
 *
 * THE PIN. \`typescript\` is pinned EXACTLY in packages/vendo/package.json (as the
 ...
 * (packages/vendo/src/apps/checking/component-screen.ts:534), and nothing else. No DOM lib, for the
 * gauntlet's own reason: \`document\` and \`fetch\` genuinely do not exist in a
 * screen, so naming one must be an error.
 *
 * Regenerate (from packages/vendo, after \`pnpm install\`):
 *   node scripts/generate-edge-lib-source.mjs
 */
...
const out = resolve(dirname(fileURLToPath(import.meta.url)), "../src/sandbox/edge/lib-source.ts");
