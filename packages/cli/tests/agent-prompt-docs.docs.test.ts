...
import { buildAgentPrompt } from "@vendoai/vendo";
import { NEXT_SERVER_EXTERNALS, SERVER_EXTERNALS_ARRAY } from "../src/framework.js";
import { compositionModuleSource } from "../src/init-scaffolds.js";
...
describe("the Next externals list the docs print is the one init writes", () => {
  ...
  it.each(NO_LIST_PAGES)("%s hands the list to init instead of printing it", async (page) => {
    ...
    expect(
      await read("packages/cli/src/init.ts"),
