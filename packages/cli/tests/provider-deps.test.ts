...
import { aiBelowPeerFloor, CLI_PACKAGE_SPEC, defaultRunner, ensureCliPackage, ensureGeneratedImports, ensureProviderDeps, ensureVendoPackage, ensureZodFloor, installCommandFor, installStderrTail, providerModuleFor, VENDO_PACKAGE_SPEC, zodBelowAiSdkFloor, ZOD_FLOOR_SPEC } from "../src/provider-deps.js";
...
/** What a package manager links for a dependency that declares a `bin` — the
    only thing a `predev` hook shelling `vendo` can actually find. */
async function linkBin(root: string, name: string): Promise<void> {
  await mkdir(join(root, "node_modules", ".bin"), { recursive: true });
  await writeFile(join(root, "node_modules", ".bin", name), "#!/usr/bin/env node\n");
}
...
// Init writes `predev`/`prebuild` hooks that shell `vendo sync` (init.ts
// SYNC_HOOKS). Since the CLI split the `vendo` bin ships in @vendoai/cli, so a
// host whose only Vendo dependency is the umbrella has hooks that call a binary
// it does not have, and its first `npm run dev` fails.

describe("ensureCliPackage", () => {
  it("adds the package that owns the `vendo` bin the hooks shell, as a dev dependency", async () => {
    const root = await tempRoot();
    await installModule(root, "@vendoai/vendo");
    await writeFile(join(root, "pnpm-lock.yaml"), "");
    const calls: Array<{ command: string; args: string[]; cwd: string }> = [];
    const messages = output();
    await ensureCliPackage({
      root,
      output: messages.sink,
      run: async (command, args, cwd) => {
        calls.push({ command, args, cwd });
    ...
    // `-D`: the hooks run at dev and build time, so the CLI stays out of the
    // production dependency closure — which is the point of the split.
    expect(calls).toEqual([{ command: "pnpm", args: ["add", "-D", CLI_PACKAGE_SPEC], cwd: root }]);
    expect(messages.logs.join("\n")).toContain(`Installed ${CLI_PACKAGE_SPEC}.`);
    expect(messages.errors).toEqual([]);
  });

  it("stays quiet when the bin is already linked (the `vendoai` alias carries it too)", async () => {
    const root = await tempRoot();
    await installModule(root, "vendoai");
    await linkBin(root, "vendo");
    const calls: unknown[] = [];
    const messages = output();
    await ensureCliPackage({ root, output: messages.sink, run: async (...call) => (calls.push(call), 0) });
    expect(calls).toEqual([]);
    expect(messages.logs).toEqual([]);
  });
  ...
  it("names the exact command to run when the install fails", async () => {
    const root = await tempRoot();
    await installModule(root, "@vendoai/vendo");
    await writeFile(join(root, "pnpm-lock.yaml"), "");
    const messages = output();
    await ensureCliPackage({ root, output: messages.sink, run: async () => 1 });
    expect(messages.errors.join("\n")).toContain(`pnpm add -D ${CLI_PACKAGE_SPEC}`);
  });
});
