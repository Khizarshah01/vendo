...
import { runInit, type InitReceipt } from "../src/init.js";
import type { InitQuestions } from "../src/init-questions.js";
import { CLI_VERSION, type Output } from "../src/shared.js";
...
function run(root: string, sink: { output: Output }, extra: Partial<Parameters<typeof runInit>[0]> = {}): Promise<number> {
  return runInit({
    ...
    // at the npm registry — `@vendoai/cli` most of all, which is not published
    // yet. Tests that assert on an install pass their own runners through
    // `extra`; the rest must never touch the network.
...
describe("what the generated files import becomes a host dependency", () => {
  ...
  it("installs @vendoai/vendo when the host's package.json never declared it", async () => {
    ...
    // Two repairs, both owed by what this run wrote: the package the generated
    // files import, and the CLI the package.json hooks shell.
    expect(installs).toHaveLength(2);
    expect(installs[0]?.args).toContain(`@vendoai/vendo@${CLI_VERSION}`);
    expect(installs[1]?.args).toEqual(["install", "-D", "@vendoai/cli"]);
  ...
  it("installs the CLI whose bin the hooks it just wrote will shell", async () => {
    // The umbrella alone stopped carrying the `vendo` bin when the CLI became
    // its own package, so a host declaring only @vendoai/vendo has predev and
    // prebuild hooks it cannot run until init adds the CLI.
    ...
    expect(installs).toEqual([["install", "-D", "@vendoai/cli"]]);
    expect(sink.logs.join("\n")).toContain("Installing the CLI your package.json hooks run");
    // The hook that needs it really was written — the claim is not canned.
