...
/** The package that owns the `vendo` bin. Deliberately NOT pinned to
    CLI_VERSION, unlike VENDO_PACKAGE_SPEC: that pin exists because the files
    init GENERATES import `@vendoai/vendo/*`, so a version skew there is a
    compile error. Nothing imports this package — the hooks only shell its bin —
    so it takes its own latest, which is this file's stated default for anything
    a scaffold does not import (`specFor`). */
export const CLI_PACKAGE_SPEC = "@vendoai/cli";

/** Is the `vendo` bin linked where a package script would find it? That is the
    only question the hooks ask: a package manager links the bins of a project's
    direct dependencies into `node_modules/.bin`, and `npm run` puts exactly
    that directory on PATH. Asking the manifest instead would miss the `vendoai`
    alias, which declares the same bin. */
async function hasVendoBin(root: string): Promise<boolean> {
  return access(join(root, "node_modules", ".bin", "vendo")).then(() => true, () => false);
}

/** The paste-ready dev install of the CLI, for the failed-repair warning. */
async function cliPackageInvocation(root: string): Promise<string> {
  return invocationFor(await installCommandFor(root), ["-D", CLI_PACKAGE_SPEC], root);
}

/**
 * Init writes `predev` and `prebuild` hooks that shell `vendo sync` (init.ts
 * SYNC_HOOKS), and since the CLI split that binary ships in `@vendoai/cli` —
 * installing `@vendoai/vendo` no longer supplies it. Writing a script that
 * calls a binary the project does not have is what broke a fresh install's
 * first `npm run dev`. Init wrote the hooks, so init makes their binary
 * resolvable, exactly as it does for the imports it authors.
 *
 * `-D`: the hooks run at dev and build time, so the CLI belongs in the dev
 * block and stays out of the production dependency closure — the point of the
 * split. A host that has installed nothing yet is skipped for the same reason
 * `ensureGeneratedImports` skips it, and a failure degrades to the exact
 * manual command like every other repair here.
 */
export async function ensureCliPackage(options: { root: string; output: Output; run?: InstallRunner }): Promise<void> {
  if (await hasVendoBin(options.root)) return;
  if (!(await hasInstalledTree(options.root))) return;

  const install = await installCommandFor(options.root);
  options.output.log(`Installing the CLI your package.json hooks run: ${CLI_PACKAGE_SPEC} (${install.command})…`);
  const code = await (options.run ?? defaultRunner)(install.command, [...install.args, "-D", CLI_PACKAGE_SPEC], install.cwd);
  if (code === 0) {
    options.output.log(`Installed ${CLI_PACKAGE_SPEC}.`);
  } else {
    options.output.error(
      `warning: could not install ${CLI_PACKAGE_SPEC} — the predev/prebuild hooks run \`vendo sync\`, which needs it, so \`npm run dev\` will fail; run \`${await cliPackageInvocation(options.root)}\` yourself.${installSaid(options.run)}`,
    );
  }
}
