...
import { aiBelowPeerFloor, ensureCliPackage, ensureGeneratedImports, ensureProviderDeps, ensureVendoPackage, ensureZodFloor, type InstallRunner } from "./provider-deps.js";
import {
...
async function ensureHostDeps(input: {
  ...
  // This run wrote `predev`/`prebuild` hooks that shell `vendo sync`
  // (SYNC_HOOKS), and that binary ships in @vendoai/cli, not in the umbrella —
  // so without this the host's very first `npm run dev` dies on `vendo: not
  // found`. Same principle as the imports above: init wrote it, init makes it
  // resolvable.
