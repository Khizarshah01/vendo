...
```text wiring/client
warning: no <VendoProvider> found in the host source — the @vendoai/vendo/ui hooks
...
`@vendoai/vendo/ui` hooks and embeds throw at runtime without that context, so
doctor flags its absence.

## The fix

If you use `@vendoai/vendo/ui` hooks or embeds anywhere, wrap the client app in
`<VendoProvider>`.
...
If your host drives a fully custom surface without `@vendoai/vendo/ui` at all,
