...
Plenty of codebases forbid deep package imports and re-export what they use through a kit module. Sync follows that: it reads your module's exports and traces `Remixable` back to `@vendoai/vendo/ui`, so wrapping works the same behind a shim.

```ts src/kit/vendo-kit.ts
export { Remixable } from "@vendoai/vendo/ui/chrome";
```
...
warning: 1 <Remixable> wrapper was found but NOT captured — sync could not trace its `Remixable` back to @vendoai/vendo/ui:
  demos/maple/Overview.tsx:4 — <Remixable> was NOT captured: `Remixable` comes from "@host/vendo-kit", and following that module's exports never reached @vendoai/vendo/ui's Remixable. Fix it either way: import it directly (`import { Remixable } from "@vendoai/vendo/ui/chrome"`), or re-export it from "@host/vendo-kit" (`export { Remixable } from "@vendoai/vendo/ui/chrome"`). Then re-run `vendo sync`.
```
