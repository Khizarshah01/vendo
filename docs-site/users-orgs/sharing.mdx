...
`appAccess` ships on `@vendoai/vendo/store`, a subpath of the package you
already have — it is not re-exported by the umbrella's root.

```ts app/api/share/route.ts highlight={5}
import { appAccess } from "@vendoai/vendo/store";
