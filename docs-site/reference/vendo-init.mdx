...
The `vendo` bin ships in `@vendoai/cli`, not in `@vendoai/vendo`. Before anything is installed, `npx vendoai@latest init` runs it straight from npm.
...
Those hooks resolve `vendo` out of `node_modules/.bin`, so `@vendoai/cli` has to stay installed in the project — not just run once through `npx`. Init installs it as a dev dependency when it writes them, so a project it wired can always run its own hooks.
