...
The `@vendoai/cli` package owns the `vendo` bin — install it alongside `@vendoai/vendo`. The `vendoai` alias re-exposes it, so `npx vendoai@latest …` runs the CLI before anything is installed. `vendo --version` prints the version, `vendo --help` the command list.
