...
Type imports come from the umbrella, not the block packages. Importing the same type from `@vendoai/vendo/core` is a `TS2307` under strict linking, because it is not your dependency.
