...
| `@vendoai/cli/extract` | The extraction stages the CLI drives |
| `@vendoai/vendo` | `agent()`, the standalone backend agent |

Type imports come from the umbrella, not the block packages. Importing the same type from `@vendoai/core` or `@vendoai/vendo/apps` is a `TS2307` under strict linking, because those are not your dependencies.
